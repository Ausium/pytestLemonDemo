...
class MyAssert:
    ...
    def assert_db(self,check_db_str):
        ...
        # 把字符串转换成python列表--因为要计算，所以现在需要用eval
        ...
        for check_db_dict in check_db_list:
            ...
            if check_db_dict["db_type"] == "count":
                ...
                logger.info("比对数据库查询的数据，是否与期望相等")
                #执行sql语句，查询结果是一个字典
                res = db.get_one_data(check_db_dict["sql"])
                logger.info("sql的执行结果为：{}".format(res))

                #对于数据库查询结果中，如果有Decimal类型，则转换为float类型
                for key,value in res.items():
                    if isinstance(value,Decimal):
                        res[key] = float[value]

            else:
                logger.info("不支持的数据库对比类型，请检查你的断言写法!!!")
                raise Exception
                # return False  可以往下走，但是看不到bug在哪，很难定位到问题
